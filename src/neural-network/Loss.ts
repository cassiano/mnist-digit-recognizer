...
export class Loss {
   ...
   */
  static crossEntropy(predicted: number[], actual: number): number {
    const epsilon = 1e-15;
   ...
   */
  static crossEntropyBatch(predictions: number[][], actuals: number[]): number {
    let totalLoss = 0;
    for (let i = 0; i < predictions.length; i++) {
      totalLoss += Loss.crossEntropy(predictions[i], actuals[i]);
    }
