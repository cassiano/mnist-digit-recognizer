...
import type { NetworkConfig, TrainingData, TrainingResult, Prediction } from './types';
...
export class Network {
  ...
  constructor(config: NetworkConfig) {
    ...
    for (let i = 0; i < layerSizes.length - 1; i++) {
      ...
      this.layers.push(
        new Layer(layerSizes[i], layerSizes[i + 1], isOutput ? 'softmax' : (config.activation || 'relu')),
      );
    }
  }
   ...
   */
  train(data: TrainingData, epochs: number, batchSize: number = 32, onEpoch?: (result: TrainingResult) => void): void {
   ...
   */
  getNetworkInfo(): { totalNeurons: number; totalWeights: number; layerDetails: { neurons: number; weights: number }[] } {
    let totalNeurons = 0;
    let totalWeights = 0;
  ...
  serialize(): string {
    const data = {
      layerSizes: this.layers.map(l => [l.inputSize, l.outputSize, l.activation]),
  ...
  static deserialize(json: string): Network {
    ...
    const sizes = [data.layerSizes[0][0], ...data.layerSizes.map((l: number[]) => l[1])];
    const net = new Network({ layers: sizes, learningRate: 0.1, activation: 'relu' });
